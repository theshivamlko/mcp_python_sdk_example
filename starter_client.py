...
class Configuration:
    ...
    def load_config(file_path: str | Path) -> dict[str, Any]:
        ...
        try:
            ...
            raise FileNotFoundError(f"Configuration file not found at {file_path}") from exc
    ...
    async def execute_tool(
        ...
        while attempt <= retries:
            try:
                ...
                attempt += 1
                if attempt > retries:
                    logging.error(f"Tool {tool_name} failed after {retries} retries: {e}")
                    raise
                logging.warning(f"Error executing {tool_name}: {e}. Retrying in {delay} seconds (attempt {attempt}/{retries}).")
...
class ChatSession:
    ...
    async def process_query(self, query: str) -> None:
        ...
        while process_query:
            ...
            tool_args = getattr(tool_use_block, 'input', {}) or {}
            server_name = self.tool_to_server.get(tool_name)
            if not server_name:
                raise RuntimeError(f"No server registered for tool {tool_name}")
            ...
            tool_result_text = self._serialize_tool_result(tool_result)
            extracted_url = self._extract_url_from_result(tool_result_text)
            if extracted_url:
